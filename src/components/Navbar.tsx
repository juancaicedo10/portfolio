...
function Navbar() {
  ...
  return (
    ...
    <div className="flex items-center select-none" onClick={() => scrollTo('home')}>
      ...
      <button data-collapse-toggle="navbar-default" type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-white rounded-lg md:hidden hover:bg-white focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-default" aria-expanded="false"  onClick={() => setIsOpen(!isOpen)}>
      ...
      <div className={`${isOpen ? '' : 'hidden'} w-full md:block md:w-auto`} id="navbar-default">
        <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 rounded-lg md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 bg-black">
          ...
          <li>
          <h1 onClick={() => scrollTo('technologies')} className="block py-2 px-3 text-white  rounded md:bg-transparent  md:p-0 dark:text-white cursor-pointer select-none text-2xl md:text-2xl">Technologies</h1>
          </li>
          <li>
          <h1 onClick={() => scrollTo('contact')} className="block py-2 px-3 text-white  rounded md:bg-transparent  md:p-0 dark:text-white cursor-pointer select-none text-2xl md:text-2xl">Contact</h1>
          </li>
        </ul>
      </div>
    </div>
